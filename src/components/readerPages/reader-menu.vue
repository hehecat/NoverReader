<template>
  <div class="reader-menu-container">
    ...
    <div v-show="isMenuShow"
         class="reader-menu-top">
      <mu-icon value="arrow_back_ios"
               @click="back()"
               style="vertical-align: middle;padding-left: 20px"></mu-icon>
    </div>
    <div v-show="isMenuShow"
         class="reader-menu-bottom fix">
      <a @click="showcatlog"
         class="reader-menu-tabs reader-menu-toc"
         href="javascript:">
        <mu-icon class="reader-icon-tabs"
                 value="book">目录</mu-icon>
      </a>
      <a @click="fontmenu"
         class="reader-menu-tabs reader-menu-font"
         href="javascript:">
        <mu-icon class="reader-icon-tabs"
                 value="settings">设置</mu-icon>
      </a>
      <a @click="nightmode"
         class="reader-menu-tabs reader-menu-night"
         href="javascript:">
        <mu-icon class="reader-icon-tabs "
                 :value="isnighticon">日/夜</mu-icon>
      </a>
      <div v-show="isFontMenuShow"
           class="reader-font-set">
        <div class="reader-font-size">
          <mt-range v-model.number="fontsize"
                    :min="14"
                    :max="24"
                    :step="2"
                    :bar-height="2">
        ...
        <ul class="reader-font-box">
          <li v-for="(Font,index) in font"
              :key="index"
              class="fontbtn"
              :class="{buttonfocus:isactivefont[index]}"
              @click="fontchange(Font.fontvalue,index)"
              :style="{fontFamily:Font.fontvalue}">{{Font.name}}</li>
        </ul>
        <ul class="reader-font-bg">
          <li :class="{readerFontBgActive:isactive[index]}"
...
export default {
  ...
  created () {
    ...
    // let num3 = localStorage.getItem('myfontsize') == 'null' ? 16 : localStorage.getItem('myfontsize');
    // this.fontsize = num3;
  },
